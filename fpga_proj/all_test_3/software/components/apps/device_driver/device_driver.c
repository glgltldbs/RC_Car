...
/*
		...
		ptr2 = mmap(NULL, ECAP_MAP_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, fd3, 0);

                        while(1){
			printf("pwm : %d, period - %d, duty = %d\n",duty,*((unsigned int*)ptr2 + ECAP_PERIOD_OFFSET),*((unsigned int*)ptr2+ECAP_DUTY_OFFSET));
			usleep(700);
			}
...
{
	int fd;
	unsigned int duty = 17000;
	unsigned int period = 34000;
	...
	{
	printf("test\n");
	*((unsigned int*)ptr +0) = duty;
	*((unsigned int*)ptr +1) = period;
	usleep(100);
	}
}
