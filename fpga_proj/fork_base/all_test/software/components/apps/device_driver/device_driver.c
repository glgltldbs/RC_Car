...
/******************************************/
            ...
            if((pid3 = fork()) == 0){
                //child3
                    ...
                    //child3 logic
						...
						if((fd5 = open(file_name, O_RDWR)) < 0) 
						{ 
	...
	ptr2 = mmap(NULL, PWM_MAP_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, fd4,0);
			while(1)
			{
				*((unsigned *)(ptr2 + PWM_TRI_OFFSET)) =0;
...
/*****************************************/
		...
		ioctl_mpu9250(fd);
		uint8_t c = readByte(fd, WHO_AM_I_MPU9250);  // Read WHO_AM_I register for MPU-9250
		...
		{  
				...
				MPU9250SelfTest(fd,SelfTest); // Start by performing self test and reporting values
				...
				calibrateMPU9250(fd,gyroBias, accelBias); // Calibrate gyro and accelerometers, load biases in bias registers
				...
				ioctl_ak8963(fd);
				uint8_t d = readByte(fd, AK8963_WHO_AM_I);  // Read WHO_AM_I register for AK8963
				printf("AK8963 I AM %x\n",d); 
				usleep(1000); 

				// Get magnetometer calibration from AK8963 ROM
				initAK8963(fd,magCalibration); printf("AK8963 initialized for active data mode....\n"); // Initialize device for active mode read of magnetometer
		...
		{  
				...
				if (readByte(fd, INT_STATUS) & 0x01) {  // On interrupt, check if data ready interrupt
				...
				if (!AHRS) {
						...
						if(delt_t > 500) {
								...
								if(SerialDebug) {
										...
										printf("X-gyro rate:%f degrees/sec\n",gx);  
										printf("Y-gyro rate:%f degrees/sec\n",gy);  
										printf("Z-gyro rate:%f degrees/sec\n",gz);  

										// Print mag values in degree/sec
										printf("X-mag field:%f mG\n",mx); 
										printf("Y-mag field:%f mG\n",my); 
										printf("Z-mag field:%f mG\n",mz); 

										tempCount = readTempData(fd);  // Read the adc values
										temperature = ((float) tempCount) / 333.87 + 21.0; // Temperature in degrees Centigrade
										// Print temperature in degrees Centigrade      
										printf("Temperature is %f degrees C \n",temperature); // Print T values to tenths of s degree C
								}
						...
						if (delt_t > 500) { // update LCD once per half-second independent of read rate
								...
								if(SerialDebug) {
										printf("ax = %f \t",(int)1000*ax);  
										printf("ay = %f \t",(int)1000*ay);  
										printf("az = %f mg\n",(int)1000*az);  
										printf("gx =%f\t ",gx);  
										printf("gy =%f\t ",gy);  
										printf("gz =%f mg \n ",gz);  
								...
								pitch *= 180.0f / M_PI;
								yaw   *= 180.0f / M_PI; 
								yaw   +=8.23;//in Seoul//-= 13.8; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
								roll  *= 180.0f / M_PI;
...
void readAccelData(int fd,int16_t * destination)
{
		...
		readBytes(fd, ACCEL_XOUT_H, 6, &rawData[0]);  // Read the six raw data registers into data array
...
void readGyroData(int fd,int16_t * destination)
{
		...
		readBytes(fd, GYRO_XOUT_H, 6, &rawData[0]);  // Read the six raw data registers sequentially into data array
		destination[0] = ((int16_t)rawData[0] << 8) | rawData[1] ;  // Turn the MSB and LSB into a signed 16-bit value
		destination[1] = ((int16_t)rawData[2] << 8) | rawData[3] ;  
		destination[2] = ((int16_t)rawData[4] << 8) | rawData[5] ; 
}
...
void readMagData(int fd,int16_t * destination)
{
		...
		if(readByte(fd, AK8963_ST1) & 0x01) { // wait for magnetometer data ready bit to be set
				readBytes(fd, AK8963_XOUT_L, 7, &rawData[0]);  // Read the six raw data and ST2 registers sequentially into data array
...
int16_t readTempData(int fd)
{
		...
		readBytes(fd, TEMP_OUT_H, 2, &rawData[0]);  // Read the two raw data registers sequentially into data array 
...
void initAK8963(int fd,float * destination)
{
		...
		usleep(10000);
		readBytes(fd, AK8963_ASAX, 3, &rawData[0]);  // Read the x-, y-, and z-axis calibration values
...
{  
		...
		writeByte(fd, PWR_MGMT_1, 0x01);  // Auto select clock source to be PLL gyroscope reference if ready else
		usleep(200000); 
		...
		writeByte(fd, SMPLRT_DIV, 0x04);  // Use a 200 Hz rate; a rate consistent with the filter update rate 
...
void MPU9250SelfTest(int fd,float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
		...
		writeByte(fd, ACCEL_CONFIG2, 0x02); // Set accelerometer rate to 1 kHz and bandwidth to 92 Hz
		...
		writeByte(fd, ACCEL_CONFIG, 0xE0); // Enable self test on all three axes and set accelerometer range to +/- 2 g
		writeByte(fd, GYRO_CONFIG,  0xE0); // Enable self test on all three axes and set gyro range to +/- 250 degrees/s
